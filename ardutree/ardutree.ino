...
SoftwareSerial antena(3, 2); // RX, TX
typedef struct {
   int *melody;
   int *durations;
   int numCount;
...
void setup() {
  ...
  Serial.begin(57600);
  Serial.println("wayra christmass 2013");
  antena.begin(57600);
  antena.println("wayra christmass 2013");
  int numTunes = 5;
  ...
  tunes[4].tempo = 100;  */
    ...
    int low = N_DS8;
    int high = 0;
    for (int j=0; j<tunes[0].numCount; j++) {
      int freq = tunes[0].melody[j];
